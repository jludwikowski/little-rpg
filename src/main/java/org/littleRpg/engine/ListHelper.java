...
public class ListHelper {
    ...
    public static void showItemList(String text, List <Item>list, boolean showBackOption){
        if(list != null) {
            list.sort((o1, o2) -> o1.getClass().getName().compareTo(o2.getClass().getName()));
            int listItemNumber = 0;
            ...
            while (itemIterator.hasNext()) {
                ...
                GameEntity nextItem = itemIterator.next();
                String[] processedClassName = nextItem.getClass().getName().split("[.]+");
                System.out.println(text + listItemNumber + " " + nextItem.description + " - " + "Price - " + ((Item) nextItem).price + " " + processedClassName[processedClassName.length-1]);
            }
        }
